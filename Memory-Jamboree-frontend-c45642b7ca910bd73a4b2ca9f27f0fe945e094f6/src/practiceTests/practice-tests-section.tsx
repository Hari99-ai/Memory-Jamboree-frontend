...
export default function PracticeTestSection() {
  ...
  const activeDisciplines = disciplines
    ?.filter((d) => d.status === 1 && desiredOrder.includes(Number(d.disc_id)))
  ...
  return (
    ...
    >
      {!testStarted && error && (
        <div className="max-w-3xl mx-auto mb-6 rounded-md border border-red-200 bg-red-50 p-4 text-red-700">
          Failed to load disciplines. Please try again later.
        </div>
      )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
            {isLoading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <Skeleton className="h-[250px] w-[250px] rounded-xl bg-gray-300" key={i} />
              ))
            ) : (
              <>
