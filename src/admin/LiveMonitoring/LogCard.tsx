...
export function CardWithLogs({
  ...
  return (
    <div className="p-6 mb-6 bg-white border shadow-lg bg-gradient-to-br from to-rose-100 rounded-2xl border-slate-200">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h4 className="text-lg font-semibold text-slate-800">{title}</h4>
      ...
      {showDetails && (
        <div className="grid grid-cols-1 gap-6 mt-6 sm:grid-cols-2 lg:grid-cols-3">
          {logs.map((log: any, index: number) =>
            log ? (
              <div key={index} className="p-4 transition-shadow border bg-slate-50 rounded-xl border-slate-200 hover:shadow-md">
                {log.img_log && (
                  <div className="relative mb-4">
                    {imageCache[`${API_BASE_URL}/${log.img_log}`] ? (
                      <img
...
export function PhoneDetectionLogs({
  ...
  // Filter logs to only show phone detections
  const phoneDetectedLogs = logs.filter(log => log?.phone_detection);

  // Extract all image URLs from filtered logs
  const imageUrls = phoneDetectedLogs
    .filter(log => log?.img_log)
    .map(log => `${API_BASE_URL}/${log.img_log}`);
      ...
      {showDetails && (
        <div className="grid grid-cols-1 gap-6 mt-6 sm:grid-cols-2 lg:grid-cols-3">
          {phoneDetectedLogs.map((log: any, index: number) => (
            <div key={index} className="p-4 transition-shadow border border-red-200 bg-red-50 rounded-xl hover:shadow-md">
              {log.img_log && (
                <div className="relative mb-4">
                  {imageCache[`${API_BASE_URL}/${log.img_log}`] ? (
                    <img
                      src={imageCache[`${API_BASE_URL}/${log.img_log}`]}
                      alt="Phone Detection Log"
                      className="object-cover w-full h-48 rounded-lg shadow-sm"
                    />
                  ) : loadingImages.has(`${API_BASE_URL}/${log.img_log}`) ? (
                    <div className="flex items-center justify-center w-full h-48 rounded-lg bg-slate-200">
...
export function PersonDetectionLogs({
  ...
  // Filter logs to only show person detections (person_status >= 1)
  const personDetectedLogs = logs.filter(log => log?.person_status >= 1);

  // Extract all image URLs from filtered logs
  const imageUrls = personDetectedLogs
    .filter(log => log?.img_log)
    .map(log => `${API_BASE_URL}/${log.img_log}`);
      ...
      {showDetails && (
        <div className="grid grid-cols-1 gap-6 mt-6 sm:grid-cols-2 lg:grid-cols-3">
          {personDetectedLogs.map((log: any, index: number) => (
            <div key={index} className="p-4 transition-shadow border border-orange-200 bg-orange-50 rounded-xl hover:shadow-md">
              {log.img_log && (
                <div className="relative mb-4">
                  {imageCache[`${API_BASE_URL}/${log.img_log}`] ? (
                    <img
                      src={imageCache[`${API_BASE_URL}/${log.img_log}`]}
                      alt="Person Detection Log"
                      className="object-cover w-full h-48 rounded-lg shadow-sm"
                    />
                  ) : loadingImages.has(`${API_BASE_URL}/${log.img_log}`) ? (
                    <div className="flex items-center justify-center w-full h-48 rounded-lg bg-slate-200">
