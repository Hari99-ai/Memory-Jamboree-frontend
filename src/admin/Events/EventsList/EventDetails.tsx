    ...
    <div className="">
      ...
      >
        <button
          ...
          disabled={!canEdit}
          // RESPONSIVE: Adjusted position and padding for mobile vs desktop
          className={`absolute top-4 right-4 md:top-10 md:right-20 bg-yellow-300 p-2 text-sm md:p-4 md:text-base rounded-xl transition-colors ${canEdit ? "cursor-pointer hover:bg-yellow-400" : "opacity-60 cursor-not-allowed"
            }`}
          onClick={() => {
            if (!canEdit) return;
            setEventEdit(true);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 md:gap-x-12 gap-y-4 md:gap-y-6 w-full text-sm md:text-base">
          <p className="text-white">
            <span className="font-semibold">Event Name:</span> {event?.event_name}
          </p>
          <p className="text-white">
            <span className="font-semibold">Event Category:</span>{" "}
            {formatCategory(Number(event?.category))}
          </p>
          <p className="text-white">
      ...
      <div className="max-w-7xl mx-auto bg-slate-100 p-4 md:p-8 rounded-md shadow-md">
        <div className="flex justify-between items-center mb-6 md:mb-10">
          <h2 className="text-xl md:text-3xl font-bold">Disciplines</h2>
          <button
            type="button"
            ...
            className={`bg-black text-white p-2 rounded-md transition ${canEdit ? "hover:bg-gray-800" : "opacity-50 cursor-not-allowed"
              }`}
            title={
              canEdit
                ? "Add discipline"
          ...
          <ul className="list-disc list-inside">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
              {event?.disciplines.map((item, index) => (
                <div
                  className="border rounded-lg bg-primary p-2 flex justify-center text-center text-white text-sm md:text-base break-words"
                  key={index}
                >
      ...
      <div className="max-w-7xl mx-auto bg-slate-100 p-4 md:p-8 rounded-md shadow-md">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-4 md:gap-0">
          <h2 className="md:text-3xl text-xl font-bold">Participants</h2>
          <div className="flex gap-x-4 w-full md:w-auto">
            <Link
              ...
              className={`bg-black text-xs md:text-lg text-white flex justify-center items-center p-2 rounded-md transition flex-1 md:flex-none ${canEdit ? "hover:bg-gray-800" : "opacity-50 cursor-not-allowed"
                }`}
            >
              <PlusIcon className="size-5 md:size-6 mr-2" /> Add Users
            </Link>
            <button
              type="button"
              ...
              className={`bg-black text-white text-xs md:text-lg flex justify-center items-center p-2 rounded-md transition flex-1 md:flex-none ${canEdit ? "hover:bg-gray-800" : "opacity-50 cursor-not-allowed"
                }`}
              title={
                canEdit
                  ? "Add category"
            ...
            >
              <PlusIcon className="size-5 md:size-6 mr-2" /> Add Category
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-y-4 mb-6">
          <div className="flex-1 w-full overflow-hidden">
            ...
            <div className="flex overflow-x-auto whitespace-nowrap border-b border-gray-200 pb-1 scrollbar-hide w-full">
              {/* Overall Tab */}
              <button
                onClick={() => setActiveCategoryId(-1)}
                className={`flex items-center flex-shrink-0 border-dotted rounded-lg border-2 gap-2 px-3 md:px-4 py-2 text-sm md:text-lg font-semibold transition-colors focus:outline-none mr-2 ${activeCategoryId === -1 || activeCategoryId === null
              ...
              >
                Overall
                <span
                  className={`px-2 py-0.5 rounded-full text-xs md:text-sm font-semibold ${activeCategoryId === -1 || activeCategoryId === null
              ...
              {normalizeCategories(category || []).map((cat) => (
                <button
                  ...
                  className={`flex items-center flex-shrink-0  gap-2 border-dotted border-2 rounded-lg px-3 md:px-4 py-2 text-sm md:text-lg transition-colors focus:outline-none mr-2 ${activeCategoryId === cat.cat_id
                    ? "border-b-2 font-extrabold border-blue-600 text-blue-600"
                    : "border-b-2 font-semibold border-transparent text-gray-500 hover:text-gray-700"
                    }`}
                >
                  {cat.category_name}
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-semibold ${activeCategoryId === cat.cat_id
                      ? "bg-blue-100 text-blue-700"
                      : "bg-gray-100 text-gray-600"
                      }`}
                  >
        ...
        {/* Table Container: Ensure horizontal scroll for table if data is wide */}
