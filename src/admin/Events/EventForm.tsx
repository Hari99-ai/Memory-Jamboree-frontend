...
import { ImageIcon, XCircle } from "lucide-react"; // Make sure these are imported
import {
  CardContent,
  CardFooter,
  CardHeader,
...
export default function EventForm() {
  ...
  const onSubmit = (values: EventFormValues) => {
    ...
    // For debugging FormData: (Do not use this in production as it consumes the FormData stream)
  ...
  return (
    <Form {...form}>
      <form
        ...
        className="w-full max-w-7xl min-h-screen mx-auto space-y-6 "
      >
        <CardHeader>
        ...
        {/* REMOVED: This larger preview div. We will use the inline smaller one.
        {previewUrl && (
          <div className="flex justify-center items-center px-6">
            <img
              src={previewUrl}
              alt="Event Preview"
              className="h-36 w-36 rounded-lg shadow-md object-cover"
            />
          </div>
        )}
        */}
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Event Image Field - THIS IS THE UPDATED SECTION */}
            <FormField
              control={form.control}
              name="eimage"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          if (file) {
                            try {
                              const resizedFile = await resizeImage(file); // Call resizeImage
                              setPreviewUrl(URL.createObjectURL(resizedFile)); // Set preview for resized image
                              onChange(resizedFile); // Update form state with resized image
                            } catch (error) {
                              console.error("Error processing image:", error);
                              alert("Failed to process image. Please try a different one.");
                              setPreviewUrl(null); // Clear preview on error
                              onChange(null); // Clear form field on error
                            ...
                            onChange(null); // Clear form field if no file
                          }
                        }}
                        className="hidden" // HIDE THE ORIGINAL INPUT
                        id="event-image-upload" // Add an ID to link with label/button
                      />

                      {/* Custom button/icon to trigger file input */}
                      <Label
                        htmlFor="event-image-upload" // Link to the hidden input
                      ...
                      >
                        <ImageIcon className="h-5 w-5" /> {/* Your image icon */}
                      ...
                      {previewUrl && (
                        <div className="relative flex items-center">
                          <img
                            src={previewUrl}
                            alt="Preview"
                            className="h-20 w-20 object-cover rounded-md border border-gray-300" // Smaller size for inline preview
                          />
                          <button
                            type="button"
                            onClick={() => {
                              setPreviewUrl(null); // Clear preview
                              onChange(null); // Clear form field
                          ...
                          >
                            <XCircle className="h-4 w-4" /> {/* Close/remove icon */}
                          </button>
                        </div>
                      )}
                    </div>
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  onChange={(e) => {
                    // Parse as local time (what user selected)
                    const localDate = new Date(e.target.value);
                    
                    // Convert to ISO string with timezone offset (handled automatically)
                    const isoString = localDate.toISOString();
                    
                    // Create new Date from ISO string (now properly timezone aware)
                    const newStartDate = new Date(isoString);
                    
                    // Validation (10 minutes from now)
                    const now = new Date();
              ...
              <FormControl>
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    // Create new Date from ISO string
        ...
        />
                  ...
                  <FormControl>
                    <MultiSelectCheckbox
                      ...
                      onSelectionChange={(selectedValues) => {
                        field.onChange(selectedValues.map(Number)); // store as numbers
                        form.trigger("disciplines"); // optional validation
                      }}
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <MultiSelectCheckbox
                      ...
                      onSelectionChange={(selectedValues) => {
                        field.onChange(selectedValues.map(Number)); // store as numbers
                        form.trigger("category"); // optional validation
                      }}
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <MultiSelectCheckbox
                      ...
                      onSelectionChange={(selectedValues) => {
                        field.onChange(selectedValues.map(Number)); // store as numbers
                        form.trigger("school_participants"); // optional validation
                      }}
          ...
          {/* <div className="mt-4 p-4 bg-gray-100 rounded">
            ...
            <p>Categories: {JSON.stringify(form.watch("category"))}</p>
            <p>Disciplines: {JSON.stringify(form.watch("disciplines"))}</p>
            <p>Schools: {JSON.stringify(form.watch("school_participants"))}</p>
          </div> */}
        </CardContent>
