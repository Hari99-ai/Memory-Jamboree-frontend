...
export default function PhoneStreamCam() {
  ...
  useEffect(() => {
    if (verified) setShowInstructions(true);
  }, [verified]);
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center">
      ...
      <button
        ...
        className="absolute bottom-0 right-3 z-50 rounded-md bg-blue-600 px-3 py-1 text-white shadow-md hover:bg-blue-700"
      >
        {cameraMode === "environment" ? "Switch to Front" : "Switch to Back"}
      </button>

      <div className="absolute left-3 top-5 rounded bg-black/60 px-3 py-1 font-bold text-white">
        {!started && !verified && "Verifying..."}
        {!started && verified && "Ready to start"}
        {started && !isMonitoring && precheckTimer > 0 && `Complete verification in ${formatTimer(precheckTimer)}...`}
      ...
      {showInstructions && verified && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 p-6 text-white">
          <div className="max-w-lg rounded bg-gray-900 p-6 text-center shadow-lg">
            <h2 className="mb-4 text-xl font-bold">Instructions</h2>
            <ul className="mb-4 list-inside list-disc space-y-2 text-left">
              <li>Place the mobile device 1 to 1.2 meters away from the user.</li>
              <li>Keep face, desk, hands, and surroundings clearly visible.</li>
              <li>Avoid strong glare or backlight on the camera.</li>
              <li>Click Start to begin precheck and wait for desktop command.</li>
            </ul>
            <button
              onClick={handleStart}
